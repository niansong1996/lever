...
# 1 otherwise # NOTE by new user: this should be the other way around
...
def eval_exec_match(
    ...
    # p_db_dir = os.path.dirname(p_db)
    # g_db_dir = os.path.dirname(g_db)
    # p_db_paths = [
    #     os.path.join(p_db_dir, basename)
    #     for basename in os.listdir(p_db_dir)
    #     if ".sqlite" in basename
    # ]
    # g_db_paths = [
    #     os.path.join(g_db_dir, basename)
    #     for basename in os.listdir(g_db_dir)
    #     if ".sqlite" in basename
    # ]
    ...
    for pred in preds:
        ...
        for p_db_path, g_db_path in ranger:
            g_flag, g_denotation = asyncio.run(exec_on_db(g_db_path, g_str))
            p_flag, p_denotation = asyncio.run(exec_on_db(p_db_path, pred))

            # we should expect the gold to be succesfully executed on the database
            assert (
                g_flag != "exception"
            ), f"gold query {g_str} has error {g_denotation} on database file {g_db_path}"
